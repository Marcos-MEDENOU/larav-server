        ...
        @foreach ($data as $resource)
            <x-filament::grid.column>
                <x-filament::section
                    ...
                    style="height: 10rem; box-shadow: 5rem"
                    >
                    <a
                        href="{{ $resource['url'] }}"
                    ...
                    >
                        @if ($resource['icon'])
                            <x-filament::icon
                               ...
                               style="height: 4rem; position: absolute ; left: 40%; bottom: 30%"
                                />
                        @endif
                        <span class="overlook-name" >{{ $resource['name'] }}</span>
                        <span class="overlook-count text-gray-600 dark:text-gray-300 absolute leading-none bottom-3 right-4 text-3xl font-bold"
                        style="position: absolute; bottom: 5% ; right: 10%">{{ $resource['count'] }}</span>
                    </a>
                </x-filament::section>
            </x-filament::grid.column>
        @endforeach
